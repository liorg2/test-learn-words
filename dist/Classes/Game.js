...
export class Game {
                ...
                if (word) {
                    ...
                    if (gameType === 'translation') {
                        ...
                        matchingTranslation = translationElements.find(tEl => {
                            ...
                            const matchesWords = ((_a = tEl.dataset.matchesWords) === null || _a === void 0 ? void 0 : _a.split(',')) || [];
    ...
    createPageSizeSelector(paginationContainer) {
        ...
        select.addEventListener('change', () => {
            ...
            if (this.score > 0 || this.failures > 0) {
                ...
                this.showCustomConfirmDialog("שינוי מספר הפריטים בדף", "שינוי מספר הפריטים בדף יתחיל את המשחק מחדש. האם להמשיך?", () => {
    ...
    showCustomConfirmDialog(title, message, onConfirm, onCancel) {
    ...
    checkPageCompletion(pageNum) {
        ...
        const allCompleted = pageWords.every(word => word.classList.contains('correct') ||
    ...
    checkAllPagesCompleted() {
        ...
        const allWordsCompleted = this.wordElements.every(word => word.classList.contains('correct') ||
            word.style.display === 'none');
    ...
    isPageEmpty(pageNum) {
        ...
        if (pageNum === this.currentPage) {
            const visibleWords = Array.from(document.querySelectorAll('#wordContainer > .word'))
                .filter((wordEl) => wordEl.offsetParent !== null && // Check if element is visible in DOM
            ...
            const visibleTranslations = Array.from(document.querySelectorAll('#targetContainer > .translation'))
                .filter((translationEl) => translationEl.offsetParent !== null &&
