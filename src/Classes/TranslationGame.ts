...
export class TranslationGame extends Game {
    ...
    organizeWordsByPage() {
        ...
        const pairs: { word: HTMLElement, translation: HTMLElement }[] = [];
        ...
        for (const wordElement of this.wordElements) {
            ...
            const matchingTranslation = this.translationElements.find(
                t => t.dataset.matchesWord === wordText
            );
        ...
        for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
            ...
            const pairsForThisPage = shuffledPairs.slice(
                pageIndex * pairsPerPage, 
                Math.min((pageIndex + 1) * pairsPerPage, shuffledPairs.length)
            );
